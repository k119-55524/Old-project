...
void Skeleton::AddMaxBone( IGameNode* boneNode, int maxBoneID )
{
	...
	newBone->SetName( boneNode->GetName() );
...
void Skeleton::WriteBone( std::ofstream& ExpFile, Bone* bone )
{
	ExpFile << bone->GetName() << " " << bone->GetID() << " " << bone->GetParentID() << std::endl;

	// Write all child bones
	bone = bone->GetChild();
	while ( bone )
	{
		this->WriteBone( ExpFile, bone );
		bone = bone->GetNext();
	}
}

void Skeleton::WriteFile( std::ofstream& ExpFile )
{
	TCHAR Text[128];

	cwNameText.SetText( "Out scelet." );
	sprintf( Text, "%d bones.", this->GetNumBones() );
	cwFaceText.SetText( Text );

	ExpFile << "[Start_Scelet_Block]\n";
	ExpFile << "Bones_Count:\n";
	ExpFile << this->GetNumBones() << std::endl;
	this->WriteBone( ExpFile, this->GetRootBone() );

	ExpFile << "[End_Scelet_Block]\n";
}
