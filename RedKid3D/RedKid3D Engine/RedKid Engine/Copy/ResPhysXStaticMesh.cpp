#include "System.h"
#include "LogFile.h"
#include "ResPhysXStaticMesh.h"

CResPhysXStaticMesh::CResPhysXStaticMesh( void )
	: CResMesh()
{
	Type = RES_PHYSX_STATIC_MESH;
...
void CResPhysXStaticMesh::Release( void )
{
	// PhysX
	DEL_ARRAY( pVerts );
	DEL_ARRAY( pIndex );
...
HRESULT CResPhysXStaticMesh::CreateMeshOfTXTFile( TCHAR* FileName, CMeshFormat* mForm )
{
	std::ifstream file;
...
HRESULT CResPhysXStaticMesh::LoadMeshBlock( std::ifstream &file, CMeshFormat* mForm, TCHAR* FileName )
{
	cMeshFormat.Release();
	...
	try
	{
		...
		meshDesc.triangles.count		= mIndexCount;			// Количесство индексов
		...
		if ( !System.GetPhysXCooking()->cookTriangleMesh( meshDesc, buf ) )
		{
			Release();
		...
		PhysXMesh = System.GetPhysX()->createTriangleMesh( MemoryReadBuffer( buf.data ) );
		if ( PhysXMesh == NULL )
		{
			Release();
		...
		//UINT cV = PhysXMesh->getNbVertices();
		//UINT cI = PhysXMesh->getNbTriangles();
		//
		///////////////////////////////////////////////////////////////////////////////////////////////
...
void CResPhysXStaticMesh::CalcAABB( D3DXMATRIX* m_GlobalWordMat, CAABB &cAABB )
{
	D3DXVECTOR3 min, max;
