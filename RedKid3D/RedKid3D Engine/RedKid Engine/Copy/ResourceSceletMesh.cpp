...
bool CResourceSceletMesh::UpdateAnimation( float DeltaTime, D3DXMATRIX* m_GlobalWordMat )
{
	...
	System.GetDeviceContext()->CSSetShaderResources( 0, 1, aRViewsSRV );

	// Устанавливаем выходной буффер
	aRViewsUAV[0] = c_UAVOut.GetBufferUAV();
	aRViewsUAV[1] = System.GetUAVBufferCalcAABB()->GetBufferUAV();
	System.GetDeviceContext()->CSSetUnorderedAccessViews( 0, 2, aRViewsUAV, NULL );

	System.GetDeviceContext()->CSSetShader( System.GetCSCopyCalcAABB_Scelet(), NULL, 0 );
	System.GetDeviceContext()->Dispatch( 1, 1, 1 );
	...
	if ( ActivAnimation != NULL )
	{
		...
		System.GetDeviceContext()->CSSetShader( g_pCSAnim, NULL, 0 );
	}
	else
		System.GetDeviceContext()->CSSetShader( g_pCSCopy, NULL, 0 );

	// Выполняем шейдер
	//for ( UINT i = 0; i < 1000; i++ )
	System.GetDeviceContext()->Dispatch( (UINT)ceil( (float)mVertexCount / 32.0f ), 1, 1 );
	...
	// Обнуляем буффера
	System.GetDeviceContext()->CSSetShaderResources( 0, 1, ppSRVNULL );
	System.GetDeviceContext()->CSSetUnorderedAccessViews( 0, 2, ppUAVNULL, NULL );
	//System.GetDeviceContext()->CSSetShader( NULL, NULL, 0 );
...
HRESULT CResourceSceletMesh::SetMaterial( CResMat* mat )
{
	...
	{
		g_pCSCopy = System.GetCSSceletTBNCopy();
		g_pCSAnim = System.GetCSSceletTBNAnim();
		StructSize = sizeof( VERTEX_TBN_UV );
	}
	else
	{
		g_pCSCopy = System.GetCSSceletNormalCopy();
		g_pCSAnim = System.GetCSSceletNormalAnim();
