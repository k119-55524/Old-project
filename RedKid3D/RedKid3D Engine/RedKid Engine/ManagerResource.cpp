...
HRESULT CManagerResource::CreatePhysXSceletMeshOfTXTFile( TCHAR* FileName, UINT IndexMat, UINT &IndexMesh )
{
	if ( FileName == NULL )
	{
		LogFile.OutputLogFileErrorString( L"NULL TXT mesh file name.", __FILE__, __FUNCTION__, __LINE__ );
	...
	if ( !aMaterial[IndexMat]->GetMeshFormat()->GetfSceletData() )
	{
		LogFile.OutputLogFileErrorString( L"The material does not contain the data about a skeleton.", __FILE__, __FUNCTION__, __LINE__ );
		return S_FALSE;
	}

	CResPhysXSceletMesh* Mesh = NULL;
	if ( S_OK != FindPhysXSceletMeshByName( FileName, IndexMat, IndexMesh ) )
	{
		Mesh = new CResPhysXSceletMesh();
		if ( Mesh == NULL )
		{
			LogFile.OutputLogFileErrorString( L"Memory allocation failure.", __FILE__, __FUNCTION__, __LINE__  );
			return S_FALSE;
...
HRESULT CManagerResource::FindPhysXSceletMeshByName( TCHAR* Name, UINT IndexMat, UINT &Index )
{
	for ( Index = 0; Index < aMesh.GetCount(); Index++ )
		if ( aMesh[Index] != NULL )
			if ( aMesh[Index]->GetType() == RES_PHYSX_SCELET_MESH )
...
HRESULT CManagerResource::CreateSceletAnimation( TCHAR* FileName, UINT &IndexAnim, UINT BoneCount )
{
	if ( S_OK != FindSceletAnimationByName( FileName, IndexAnim ) )
	{
		...
		V_RETURN( Anim->CreateAnimationOfTXTFile( FileName, BoneCount ) );
		...
		if ( aSceletAnim[IndexAnim]->GetBoneCount() != BoneCount )
		{
			TCHAR mESS[100];
