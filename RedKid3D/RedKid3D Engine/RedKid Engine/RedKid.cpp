...
HRESULT CRedKid::InicialiseForEditor( HWND hWnd, PxDefaultAllocator &PhysXDefAlloc, PxErrorCallback &PhysXDefError )
{
	if ( SystemRedKid.GetfInit() )
		return S_FALSE;

	V_RETURN( SystemRedKid.InicialiseForEditor( hWnd, PhysXDefAlloc, PhysXDefError, D3D_FEATURE_LEVEL_11_0 ) );
	...
	try
	{
		...
		V_RETURN( SystemRedKid.Resize() );
	}
	catch( ... )
	{
		LogFile.OutputLogFileErrorString( L"Unknown error inicialise RedKid3D for editor(catch ... RedKid class).", __FILE__, __FUNCTION__, __LINE__  );
		return S_FALSE;
	}
...
HRESULT CRedKid::Inicialise( HINSTANCE hInstance, PxDefaultAllocator &PhysXDefAlloc, PxErrorCallback &PhysXDefError, D3D_FEATURE_LEVEL cLev )
{
	...
	try
	{
		...
		ActivRenderPool = RenderPool;
			...
			PxExtensionVisualDebugger::connect( SystemRedKid.GetPhysX()->getPvdConnectionManager(), "localhost", 5425, 10000, true );
		#endif
...
HRESULT CRedKid::AddPhysXTriangleStaticMeshObject( CObjectBase* &Obj, TCHAR* Name, TCHAR* FileName, UINT IndexMat, CObjectBase* ParentObj, bool fRenderSign )
{
	...
	if ( IndexMat >= cResManager.aMaterial.GetCount() || cResManager.aMaterial[IndexMat] == NULL )
	{
		LogFile.OutputLogFileErrorString( _T( "Not a correct index of a material." ), __FILE__, __FUNCTION__, __LINE__ );
		return S_FALSE;
	}

	if ( cResManager.aMaterial[IndexMat]->GetMeshFormat()->GetfSceletData() )
	{
		LogFile.OutputLogFileErrorString( _T( "Scelet data in material." ), __FILE__, __FUNCTION__, __LINE__ );
		return S_FALSE;
	}
	...
	if ( S_OK != ((CObjectPhysX*)Obj)->AddMesh( cResManager.aMesh[IndexMesh], ActivRenderPool->GetPhysXScene() ) )
	{
		delete Obj;
...
HRESULT CRedKid::AddPhysXSceletObject( CObjectBase* &Obj, TCHAR* Name, TCHAR* FileName, TCHAR* FileAnimName, UINT IndexMat, CObjectBase* ParentObj, bool fRenderSign )
{
	Obj = NULL;
	...
	if ( !cResManager.aMaterial[IndexMat]->GetMeshFormat()->GetfSceletData() )
	{
		LogFile.OutputLogFileErrorString( _T( "Not scelet data in material." ), __FILE__, __FUNCTION__, __LINE__ );
		return S_FALSE;
	}

	UINT IndexMesh;
	V_RETURN( cResManager.CreatePhysXSceletMeshOfTXTFile( FileName, IndexMat, IndexMesh ) );

	UINT IndexAnim;
	V_RETURN( cResManager.CreateSceletAnimation( FileAnimName, IndexAnim, cResManager.aMesh[IndexMesh]->GetBoneCount() ) );

	Obj = new CObjectPhysXSceletMesh( Name );
	if ( Obj == NULL )
	{
		LogFile.OutputLogFileErrorString( L"Memory allocation failure.", __FILE__, __FUNCTION__, __LINE__  );
		return S_FALSE;
	...
	if ( S_OK != ((CObjectPhysXSceletMesh*)Obj)->AddMesh( cResManager.aMesh[IndexMesh], ActivRenderPool->GetPhysXScene(), cResManager.aSceletAnim[IndexAnim] ) )
	{
		delete Obj;
		LogFile.OutputLogFileErrorString( L"Error AddMesh.", __FILE__, __FUNCTION__, __LINE__  );
		return S_FALSE;
