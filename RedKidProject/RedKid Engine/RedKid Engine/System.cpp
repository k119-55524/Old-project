...
void CSystem::DefferedRenderLight( float ElapsedTime, D3DXMATRIX* mWVInv, CArrayBufferStruct<LIGHT_POINT> *aLightPoint, CArrayBufferStruct<LIGHT_DIRECTION> *aLightDirection )
{
	...
	//ppRT[0] = cTextLightRender.GetTextureRT();
	//g_pImmediateContext->OMSetRenderTargets( 2, ppRT, NULL );

	//g_pImmediateContext->IASetPrimitiveTopology( D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP );
	//g_pImmediateContext->IASetInputLayout( g_pInputLayoutQuad );
	//g_DiffuseTexture->SetResource( NULL );
	//g_pTechRenderLight->GetPassByIndex( 0 )->Apply( 0, g_pImmediateContext );

	//aRViewsSRV[0] = cTextDiff.GetTextureSRV();
	//aRViewsSRV[1] = cTextGBuffer.GetTextureSRV();
	//aRViewsSRV[2] = cTextMaterial.GetTextureSRV();
	//g_pImmediateContext->PSSetShaderResources( 0, 3, aRViewsSRV );
	...
	//c_CBRenderLight.PSSetConstantBuffers( 0 );

	//V_( aLightPoint->CopyDataOfCPUInGPU() );
	//aLightPoint->PSSetConstantBuffers( 1 );

	//V_( aLightDirection->CopyDataOfCPUInGPU() );
	//aLightDirection->PSSetConstantBuffers( 2 );

	//static UINT offset = 0;
	//static UINT StructSize = sizeof( VERTEX_QUAD );
	//g_pImmediateContext->IASetVertexBuffers( 0, 1, &g_pVBQuad, &StructSize, &offset );

	//g_pImmediateContext->Draw( 4, 0 );

	//g_pImmediateContext->PSSetShaderResources( 0, 3, ppSRVNULL );
	//g_pImmediateContext->PSSetConstantBuffers( 0, 3, ppBufferNULL );
	//g_pImmediateContext->OMSetRenderTargets( 1, ppRT, g_pDepthStencilView );
